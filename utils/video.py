...
__all__ = ["VideoGenerator", "preview_video", "preview_frame", "save_video", "display_video"]


class VideoGenerator(Dataset):
    def __init__(self, video_root, video=False, start=1, fill=5, prefix="jpg", max_frames=None):
    ...
    def _read_frames(self, start=0, fill=5, prefix="png"):
        ...
        def read_frame(idx):
            ...
            return Image.open("%s/%s" % (self.video_root, frame_name)).convert("RGB")
        with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
            frames = list(executor.map(lambda x: read_frame(x), np.arange(max_frames)))
        return frames
